...
function applyServiceState() {
  ...
  $('#btn-stop-bot').prop('disabled', !serviceState.tradingOpen)
...
function updateBotTooltip(selector, message) {
...
function updateBotControls(alias, assetMap) {
  ...
  const fundsOk = hasAnyFunds(assetMap)
  const requirementsMet = aliasSet && fundsOk
  const $start = $('#btn-start-bot')
  const $stop = $('#btn-stop-bot')

  const blockMessage =
    !aliasSet && !fundsOk
      ? 'Alias is not set and no wallet funds detected.'
      : !aliasSet
      ? 'Alias is not set.'
      : !fundsOk
  ...
  if (serviceState.tradingOpen) {
    ...
    $start.prop('disabled', !requirementsMet)
  }

  const startMessage = serviceState.tradingOpen
    ? 'Bot is already running.'
    : requirementsMet
    ? 'Start market maker bot.'
    : blockMessage || 'Alias and wallet funds required.'
  const stopMessage = serviceState.tradingOpen
    ? 'Stop market maker bot.'
    : requirementsMet
    ? 'Bot is not running.'
    : blockMessage || 'Alias and wallet funds required.'

  updateBotTooltip('#start-bot-tooltip', startMessage)
  updateBotTooltip('#stop-bot-tooltip', stopMessage)
}
...
$(function () {
  ...
  $('#btn-start-bot').on('click', function () {
    ...
    updateBotTooltip('#start-bot-tooltip', 'Starting bot…')
    $.ajax({
      url: '/api/bot/start',
      method: 'POST',
      contentType: 'application/json',
      ...
      .done(function () {
        ...
        updateBotTooltip('#start-bot-tooltip', 'Bot started.')
      })
      .fail(function (jqXHR) {
        updateBotTooltip(
          '#start-bot-tooltip',
          'Failed to start bot (' + jqXHR.status + ').'
        )
      })
      .always(function () {
        $btn.text('Start Bot')
  ...
  $('#btn-stop-bot').on('click', function () {
    ...
    $('#btn-start-bot').prop('disabled', true)
    updateBotTooltip('#stop-bot-tooltip', 'Stopping bot…')
    $.ajax({
      url: '/api/bot/stop',
      method: 'POST',
      contentType: 'application/json',
      ...
      .done(function () {
        serviceState.tradingOpen = false
        updateBotTooltip('#stop-bot-tooltip', 'Bot stopped.')
      })
      .fail(function (jqXHR) {
        updateBotTooltip(
          '#stop-bot-tooltip',
          'Failed to stop bot (' + jqXHR.status + ').'
        )
      })
      .always(function () {
        $btn.text('Stop Bot')
